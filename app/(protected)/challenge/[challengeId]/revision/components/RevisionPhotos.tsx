  ...
  const uploadFile = async (e: any, photoType: PhotoType) => {
    ...
    if (file !== undefined) {
      ...
      const response = await fetch(
        'https://api.cloudinary.com/v1_1/dsnui81gh/image/upload',
        {
          method: 'POST',
          body: formData,
        },
      ...
      if (!response?.ok) {
        ...
        return toast({
          ...
          description:
            'La imagen no ha podido añadirse. Por favor, inténtalo de nuevo.',
          variant: 'destructive',
        });
      }
      ...
      const apiResponse = await fetch(
        `/api/challenge/${revision.challengeId}/revision/${revision.id}/photo`,
        {
          method: 'POST',
          headers: {
