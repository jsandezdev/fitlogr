...
const newWorkoutStepWeightFormSchema = z.object({
  weightGoal: weightGoalSchema,
});

type NewWorkoutStepWeightFormValues = z.infer<
  typeof newWorkoutStepWeightFormSchema
>;

const defaultValues: Partial<NewWorkoutStepWeightFormValues> = {
  weightGoal: {
    type: undefined,
    amount: 0,
    frequency: undefined,
...
type Props = {
  onNext: (formData: NewWorkoutStepWeightFormValues) => void;
  onPrevious: () => void;
};

export const NewWorkoutStepWeightForm = ({ onNext, onPrevious }: Props) => {
  const [isLoading, setIsLoading] = useState<boolean>(false);

  const form = useForm<NewWorkoutStepWeightFormValues>({
    resolver: zodResolver(newWorkoutStepWeightFormSchema),
    defaultValues,
  });

  async function onSubmit(formData: NewWorkoutStepWeightFormValues) {
    setIsLoading(true);
    onNext(formData);
    setIsLoading(false);
  }
