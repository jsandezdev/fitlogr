...
const newWorkoutStepBodyPartsFormSchema = z.object({
  bodyPartGoals: z.array(bodyPartGoalSchema),
});

type NewWorkoutStepBodyPartsFormValues = z.infer<
  typeof newWorkoutStepBodyPartsFormSchema
>;

const defaultValues: Partial<NewWorkoutStepBodyPartsFormValues> = {
  bodyPartGoals: [
    {
...
type Props = {
  onNext: (formData: NewWorkoutStepBodyPartsFormValues) => void;
  onPrevious: () => void;
};

export const NewWorkoutStepBodyPartsForm = ({ onNext, onPrevious }: Props) => {
  const [isLoading, setIsLoading] = useState<boolean>(false);

  const form = useForm<NewWorkoutStepBodyPartsFormValues>({
    resolver: zodResolver(newWorkoutStepBodyPartsFormSchema),
    defaultValues,
  });
  ...
  async function onSubmit(formData: NewWorkoutStepBodyPartsFormValues) {
    setIsLoading(true);
    onNext(formData);
    setIsLoading(false);
  }
