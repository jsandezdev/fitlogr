...
async function deleteExercise({ id }: DeleteExerciseProps) {
  ...
  if (!response?.ok) {
    toast({
      ...
      description:
        'Tu ejercicio no ha sido eliminado. Por favor, inténtalo de nuevo.',
      variant: 'destructive',
    });
  }
...
export function ExerciseOperations({ exercise }: Props) {
    ...
    <>
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end">
          <DropdownMenuItem>
            <Link href={`/exercise/${exercise.id}`} className="flex w-full">
              Editar
            </Link>
          </DropdownMenuItem>
          <DropdownMenuSeparator />
